...
Validation for publication: `npm run verify` passed on September 22, covering environment documentation, ESLint, Prisma generation, TypeScript, and the production build. The earlier workflow and browser checks are recorded in [the launch report](launch-2026-09-18.md). Environment files, credentials, local uploads, and generated build/test output are excluded. Sanitized review evidence is included. A documentation example that matched the local authentication secret was replaced with a placeholder. GitHub verification now supplies a build-only loopback database URL because private environment files are no longer tracked; it receives no production database credentials.
