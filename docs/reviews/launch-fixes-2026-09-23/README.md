...
`npm run verify` passed (environment documentation, lint, Prisma generation, optimized production build). Thirty-eight distinct focused checks passed: 16 in `tests/launch-workflow-fixes.spec.ts`, 17 existing account-recovery checks, four checkout/quote regressions, and one admin API/UI regression. Tests ran against localhost:3150 and the isolated local `phenofarm_auth_launch_20260923` database, never the ordinary development database or production. Separate scoped production checks used four synthetic accounts and explicit production-target guards.
...
- Production mail settings are active. `AUTH_REQUIRE_EMAIL_VERIFICATION=true` deliberately requires existing unverified users to confirm their email before signing in; no account is marked verified without proof. Verification and password reset remain available to those users.
...
- Real-user verification and reset emails were accepted by the live app and both have a `delivered` provider event. The user's initial sender test was confirmed in the Yahoo inbox. Their own email verification remains a user action; this audit did not change their password or mark them verified. Synthetic signup/reset messages use Resend's documented labeled test recipients, with links retrieved privately from the sending provider; these checks are distinct from real inbox placement.
...
- Final code release: `dpl_HRUCjWBHi1UU7XN79ieRe2utM6sP`, https://phenofarm-8wxiiqpg6-kevin-means-projects.vercel.app, Ready and aliased to the production URL. Source commit: `26b978d8e9f649f508e98f631190c42d68b610da`. The final live mobile layout and original-receipt replay passed after this deployment.
- All 20 scoped live checks passed. All four synthetic accounts and their related products, orders, quotes, conversations, receipts, and notifications were removed. Every one of the 51 existing business records across 19 tables matches the pre-test count/hash snapshot. No Blob objects were uploaded in this remediation pass. Auth rate-limit/action-token tables and migration history are excluded from the business-record comparison because real verification/reset requests and the additive migration intentionally affect them.
- Production has 23 completed migrations, including the new order-request receipt table. The recovery branch remains available.

## Final evidence

- [Live check results](live-checks.json)
- [Release, delivery, migration, and cleanup verification](live/release-verification.json)
- [Business records before](live/baseline.json) and [after cleanup](live/after-cleanup.json)
- [Final mobile order layout](live/live-settlement-dispensary-final.png) and [notes visible above the mobile actions](live/live-settlement-visible-mobile.png)
- [Live recovered request](live/live-request-recovered-mobile.png), [password reset success](live/live-reset-success-mobile.png), and [settlement CSV](live/live-settlement-dispensary.csv)

No secrets, passwords, reset links, or session cookies are included in the evidence. The real user's account/password was unchanged; completing their emailed ownership verification remains their action.
