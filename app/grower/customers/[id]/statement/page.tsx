...
export default async function CustomerStatementPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return (
    <div className="space-y-4 pb-8 sm:space-y-5">
      <Link href={`/grower/customers/${id}/edit`} className="text-sm font-semibold text-green-700">← Customer</Link>
      ...
      <form className="grid grid-cols-2 items-end gap-3 rounded-xl border border-gray-200 bg-white p-4 sm:flex sm:flex-wrap">
        <label className="min-w-0 text-sm font-medium">From<input type="date" name="from" defaultValue={filters.from} className="mt-1 block h-10 min-w-0 w-full text-base sm:text-sm rounded-lg border border-gray-300 px-2" /></label>
        <label className="min-w-0 text-sm font-medium">To<input type="date" name="to" defaultValue={filters.to} className="mt-1 block h-10 min-w-0 w-full text-base sm:text-sm rounded-lg border border-gray-300 px-2" /></label>
        <button className="h-10 rounded-lg bg-green-700 px-4 text-sm font-semibold text-white">Apply</button>
      </form>
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <div className="flex flex-wrap justify-between gap-3 border-b p-4 text-sm"><span className="font-semibold">{orders.length} delivered request{orders.length === 1 ? '' : 's'}</span><span className="font-bold text-green-700">{formatProductMoney(total)}</span></div>
        {orders.length ? <>
          <div className="divide-y divide-gray-200 sm:hidden">{orders.map(order => <article key={order.id} className="p-3 text-sm">
            <Link href={`/grower/orders/${order.id}`} className="break-all font-semibold text-green-700">#{order.orderId}</Link>
            <p className="mt-1 text-xs text-gray-500">{order.deliveredAt ? format(order.deliveredAt, 'MMM d, yyyy') : 'Date not recorded'}</p>
            <div className="mt-3 flex items-start justify-between gap-3"><p className="min-w-0 text-gray-600">{order.items.map(item => `${item.product?.name || 'Product'} × ${item.quantity}`).join(', ')}</p><strong className="shrink-0">{formatProductMoney(Number(order.totalAmount))}</strong></div>
          </article>)}</div>
          <div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[720px] text-sm"><thead className="bg-gray-50"><tr><th className="p-3 text-left">Request</th><th className="p-3 text-left">Delivered</th><th className="p-3 text-left">Items</th><th className="p-3 text-right">Value</th></tr></thead><tbody>{orders.map(order => <tr key={order.id} className="border-t"><td className="p-3"><Link href={`/grower/orders/${order.id}`} className="font-semibold text-green-700">#{order.orderId}</Link></td><td className="p-3">{order.deliveredAt ? format(order.deliveredAt, 'MMM d, yyyy') : 'Not recorded'}</td><td className="p-3">{order.items.map(item => `${item.product?.name || 'Product'} × ${item.quantity}`).join(', ')}</td><td className="p-3 text-right font-semibold">{formatProductMoney(Number(order.totalAmount))}</td></tr>)}</tbody></table></div>
        </> : <p className="p-8 text-center text-sm text-gray-500">No delivered requests in this date range.</p>}
      </section>
      <p className="text-xs text-gray-500">Payments stay between businesses; PhenoFarm does not collect funds.</p>
    </div>
  );
}
