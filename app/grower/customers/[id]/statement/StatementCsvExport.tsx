...
export function StatementCsvExport({ name, rows }: { name: string; rows: string[][] }) {
  ...
  return <button type="button" onClick={download} className="inline-flex h-10 items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-800 hover:bg-gray-50 focus-visible:ring-2 focus-visible:ring-green-600"><Download className="h-4 w-4" />Export CSV</button>;
}
