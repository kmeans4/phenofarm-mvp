...
export function ClientNav({ links }: { links: NavLink[] }) {
  ...
  return (
    <nav className="px-3 py-2 lg:px-4 lg:py-3 space-y-0.5 lg:space-y-1">
      {links.map((link) => {
        ...
        return (
          <Link 
            key={link.href}
            href={link.href}
            className={`flex items-center justify-between px-3 py-2.5 lg:px-4 lg:py-2.5 rounded-lg transition-colors text-sm ${
              active 
