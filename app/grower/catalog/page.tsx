...
export default async function GrowerCatalogWorkspacePage() {
  ...
  const healthSummaryItems = [
    ...
    },
    {
      ...
      accent: 'bg-pf-info-bg text-pf-info ring-pf-info-line',
    },
    {
      title: 'Low stock',
      ...
      accent: 'bg-pf-danger-bg text-pf-danger ring-pf-danger-line',
    },
  ];
  ...
  return (
    <div className="space-y-3 sm:pb-20 sm:space-y-5">
      <PageHeader mobileInlineActions title="Catalog" actions={<Link href="/grower/products/add" className="inline-flex h-10 items-center justify-center rounded-lg bg-emerald-500 px-3 text-sm font-semibold text-[#032116] hover:bg-emerald-400 sm:px-4">Add product</Link>} />
      <div className={`${totalProducts > 0 && lowStockProducts === 0 ? 'hidden sm:flex' : 'flex'} flex-wrap items-center justify-between gap-3 rounded-xl border border-pf-accent-line bg-pf-accent-bg p-3 text-sm`}>
      ...
      <div className="grid gap-2 sm:grid-cols-2 sm:gap-3">
        {workspaceSections.map(section => <Link key={section.title} href={section.href} className="min-h-12 rounded-xl border border-pf-line bg-pf-surface px-3 py-3 shadow-sm hover:border-pf-accent-line sm:p-4">
          <div className="flex items-center justify-between gap-3"><h2 className="font-semibold">{section.title}</h2><span className="shrink-0 whitespace-nowrap text-xs font-medium text-pf-accent">{section.metric}</span></div>
          <p className="mt-1 hidden text-sm text-pf-muted sm:block">{section.description}</p>
        </Link>)}
      </div>
      ...
      <details className="rounded-xl border border-pf-line bg-pf-surface px-3 py-1 shadow-sm sm:p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3">{advancedSections.map(section => <Link key={section.title} href={section.href} className="rounded-lg border border-pf-line p-3 text-sm font-medium text-pf-accent hover:bg-pf-accent-bg">{section.title} →</Link>)}</div>
      </details>
    </div>
  );
}
